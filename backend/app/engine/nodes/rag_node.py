...
class RAGRetrieveNode(BaseNode):
    async def execute(self, input_data: dict[str, Any]) -> dict[str, Any]:
        ...
        # Get scripts from the YAML scripts field (ordered dict)
        scripts: dict[str, str] = self.spec.scripts
        if not scripts:
            # Fallback: discover from organized path
            scripts = self._discover_scripts()

        step_names = list(scripts.keys())

        # Emit step names upfront so the CLI can show pending steps
        ...
        # Initial state
        state = {
            "dataset_path": str(Path("data") / dataset_name) if dataset_name else "data",
            "query": query,
            "top_k": self.params.get("top_k", 5),
            **input_data,
        }

        # Run each script in YAML definition order
    ...
    async def _run_script(
        ...
        try:
            ...
            # Lifecycle: instantiate -> validate -> run -> log -> evaluate
